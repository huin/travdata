...
impl ESTransformer {
    ...
    pub fn load_module(&mut self, script: ESScript) -> Result<()> {
        let (result_send, result_recv) = mpsc::channel();
        self.request_send.send(Request::LoadModule {
            script,
            result_send,
        })?;
    ...
    pub fn transform(&mut self, script: ESScript, tables: Vec<Table>) -> Result<Table> {
        let (result_send, result_recv) = mpsc::channel();
        ...
        while let Ok(request) = request_recv.recv() {
            ...
            match request {
                Request::LoadModule {
                    ...
                    let result: Result<()> = Self::handle_load_module(&mut ctx_scope, script);
                ...
                Request::Transform {
                    ...
                    let result = Self::handle_transform(global, &mut ctx_scope, script, tables);
                    if result_send.send(result).is_err() {
                        // TODO: log this error?
                    }
                }
    ...
    fn handle_load_module(ctx_scope: &mut CtxScope<'_, '_>, script: ESScript) -> Result<()> {
        ...
        if !result.is_undefined() {
            bail!(
                    "loading module script resulted in a value other than undefined, got value of type {}",
                    result
    ...
    fn handle_transform(
        ...
        // if script.origin.is_module {
        //     bail!("modules cannot be used to transform tables");
        // }

        let origin_v8 = script.origin.make_origin(&mut try_catch)?;
        let source_str_v8 = new_v8_string(&mut try_catch, &script.source)
            .with_context(|| "could not create source code string")?;
        let mut source_v8 = v8::script_compiler::Source::new(source_str_v8, Some(&origin_v8));
...
mod test {
    ...
    use crate::extraction::tableextract::estransform::ESScriptOrigin;
    ...
    #[test]
    fn test_load_module_as_script() {
        let mut estrn = ESTransformer::new();
        let result = estrn.load_module(new_script("const foo = {};"));
        assert_that!(result, ok(eq(())));
    }

    #[test]
    fn test_load_module_as_script_syntax_error() {
        let mut estrn = ESTransformer::new();
        let result = estrn.load_module(new_script("I'm invalid ECMAScript!"));
        assert_that!(result, err(anything()));
    }

    #[test]
    fn test_transform_table_with_loaded_module_as_script() {
        let mut estrn = ESTransformer::new();

        let module_result = estrn.load_module(new_script(
            r#"
        ...
        assert_that!(module_result, ok(anything()));

        let result = estrn.transform(
            new_script(r#"return concatTableData(tables);"#),
            vec![
